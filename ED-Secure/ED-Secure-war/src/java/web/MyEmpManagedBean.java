...
import session.EmployeeManagement;
...
//        return (salary > 0.0);
    ...
    public String setEmployeeDetailsForChange() {
        // check empId is null
        if (validateRecord() || conversation == null) {
            return "debug";
        }
    ...
    private boolean validateRecord() { // validate password
        ...
        if (isNull(empId)) {
            ...
            try {
                if (password != null) {
                    EmployeeDTO e = employeeManagement.getEmployeeDetails(empId); // Logged in as user
                    if (e == null) {
                        result = true;
                    }
                    if (e.getPassword().equals(ConvertToHexString(GenerateSHA(password)))) { // Checks if the password is the same
                        result = false;
                    } 
                    else {
                        result = true;
